...
import com.example.ioasysmvvm.domain.use_case.DoLogin
import com.example.ioasysmvvm.domain.use_case.ValidateUserEmail
import com.example.ioasysmvvm.domain.use_case.ValidateUserPassword
    ...
    fun doLogin(email: String, password: String) {
        viewModelScope.launch(dispatcher) {
            ...
            if (isValidEmail == EmailStatus.VALID && isValidPassword == PasswordStatus.VALID) {
                ...
                when (val result = doLogin.call(typedUser)) {
                    is Result.Error -> {
